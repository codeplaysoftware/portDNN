 ...
 */
#ifndef PORTDNN_BENCH_MATMUL_BENCHMARK_CONFIG_H_
#define PORTDNN_BENCH_MATMUL_BENCHMARK_CONFIG_H_
...
/**
 * Provide a set of matmul benchmark configurations.
 *
 ...
 * the benchmark State. A set of MatmulParams can be constructed from this
 * State using benchmark_params::deserialize.
 *
 ...
 */
auto RunForAllMatmulParamSets = [](benchmark::internal::Benchmark* b) {
  for (auto& config : matmul_benchmark_configs()) {
    b->Args(config);
  }
};
}  // namespace

#endif  // PORTDNN_BENCH_MATMUL_BENCHMARK_CONFIG_H_
