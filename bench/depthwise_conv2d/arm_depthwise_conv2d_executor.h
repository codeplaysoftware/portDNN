 ...
 */
#ifndef PORTDNN_BENCH_DEPTHWISE_CONV2D_ARM_CONV2D_EXECUTOR_H_
#define PORTDNN_BENCH_DEPTHWISE_CONV2D_ARM_CONV2D_EXECUTOR_H_
...
struct ACLNeonExecutor {
  ...
  arm::NEDepthwiseConvolutionLayer dconv1;
};
...
struct ACLOpenCLExecutor {
  ...
  arm::CLDepthwiseConvolutionLayer dconv1;
};

/** Executor to perform the depthwise conv2d benchmark using ARM Compute
 * Library. */
template <typename Benchmark, typename ACLExecutor>
struct ARMDepthwiseConv2DExecutor : public BaseExecutor {
 private:
 ...
 public:
  ...
  void execute(State& state, DepthwiseConv2DParams const& params) {
    if (params.channel_multiplier != 1) {
      state.SkipWithError(
          "Channel multiplier must be one for ACL depthwise convolutions");
    }
    // Allocate tensors.
    ACLExecutor ex;
    ...
    ex.bias.allocator()->init(
        arm::TensorInfo(arm::TensorShape(params.channels), arm::Format::F32));
    ...
    ex.dconv1.configure(&ex.input, &ex.filter, &ex.bias, &ex.output, psi);

    // Validate the configuration.
    auto status = ex.dconv1.validate(ex.input.info(), ex.filter.info(),
...
#endif  // PORTDNN_BENCH_DEPTHWISE_CONV2D_ARM_CONV2D_EXECUTOR_H_
