...
/**
 * Helper to convert form MemObject to BufferMemObject.
 * TODO: Remove once USM is fully supported.
 *
 * \param mo The MemObject to convert to BufferMemObject.
 * \return The converted BufferMemObject.
 */
template <typename T, typename Alloc>
BufferMemObject<T> mo_to_bo(MemObject<T, Alloc> mo) {
  return BufferMemObject<T>(mo.get_buffer(), mo.get_extent(), mo.get_offset());
}
