...
#include "src/batchnorm/gradient/frozen/queue_gamma_gradient.h"
 ...
 */
template <typename T>
SNNStatus launch_gamma_gradient(BaseMemObject<T const>& gradient,
  ...
  if (can_use_vector_width(n_items, 4)) {
    return queue_gamma_gradient<T, int32_t, 4>(gradient, input, mean, variance,
                                               output, n_items, epsilon, queue);
  } else if (can_use_vector_width(n_items, 2)) {
    return queue_gamma_gradient<T, int32_t, 2>(gradient, input, mean, variance,
                                               output, n_items, epsilon, queue);
  } else {
    return queue_gamma_gradient<T, int32_t, 1>(gradient, input, mean, variance,
                                               output, n_items, epsilon, queue);
  }
}
