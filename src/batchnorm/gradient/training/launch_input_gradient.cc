...
#include "src/batchnorm/gradient/training/queue_input_gradient.h"
 ...
 */
template <typename T>
SNNStatus launch_input_gradient(BaseMemObject<T const>& gamma,
  ...
  if (can_use_vector_width(n_items, 4)) {
    return queue_input_gradient<T, int32_t, 4>(gamma, variance, mean, x_offset,
                                               output, n_items, epsilon, queue);
  } else if (can_use_vector_width(n_items, 2)) {
    return queue_input_gradient<T, int32_t, 2>(gamma, variance, mean, x_offset,
                                               output, n_items, epsilon, queue);
  } else {
    return queue_input_gradient<T, int32_t, 1>(gamma, variance, mean, x_offset,
                                               output, n_items, epsilon, queue);
  }
}
