...
SNNStatus queue_kernel(BaseMemObject<T const>& input_mem,
                       BaseMemObject<T>& output_mem, int batches, int outer,
                       int inner, cl::sycl::queue& queue) {
  auto event = queue.submit([&](cl::sycl::handler& cgh) {
    auto input = input_mem.read_accessor(cgh);
    auto output = output_mem.write_accessor(cgh);

    using Functor = ReduceKernel<T, Index, Op>;

    Functor functor{input, output, batches, outer, inner};
