 ...
 */
#ifndef PORTDNN_SRC_BACKEND_SNN_USM_BACKEND_PROVIDER_H_
#define PORTDNN_SRC_BACKEND_SNN_USM_BACKEND_PROVIDER_H_
...
/** Specialisation of the backend provider for the SNNUSMBackend.  */
template <>
struct BackendProvider<SNNUSMBackend> {
 public:
  template <typename T>
  using Pointer = SNNUSMBackend::pointer_type<T>;
  ...
  /** Return this backend. */
  SNNUSMBackend& get_backend() { return backend_; }
  ...
  Pointer<T> get_initialised_device_memory(size_t size,
    ...
    // TODO: use copy over memcpy when ComputeCpp implements overload
    auto gpu_ptr = backend_.allocate<T>(size);
    auto event = backend_.get_queue().submit([&](cl::sycl::handler& cgh) {
      cgh.memcpy(gpu_ptr, data.data(), sizeof(T) * size);
    });
  ...
  void copy_device_data_to_host(size_t size, Pointer<T> gpu_ptr,
    ...
    auto event = backend_.get_queue().submit([&](cl::sycl::handler& cgh) {
      cgh.memcpy(host_data.data(), gpu_ptr, sizeof(T) * size);
    });
    event.wait_and_throw();
  }
  ...
  void deallocate_ptr(Pointer<T> ptr) {
    backend_.deallocate<T>(ptr);
  }

 private:
  /** The backend that this provides. */
  SNNUSMBackend backend_;
...
#endif  // PORTDNN_SRC_BACKEND_SNN_USM_BACKEND_PROVIDER_H_
