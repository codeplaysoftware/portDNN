 ...
 */
#ifndef PORTDNN_SRC_BACKEND_CLBLAST_BACKEND_PROVIDER_H_
#define PORTDNN_SRC_BACKEND_CLBLAST_BACKEND_PROVIDER_H_
...
/** Specialisation of the backend provider for the CLBlastBackend.  */
template <>
struct BackendProvider<CLBlastBackend> {
 public:
  template <typename T>
  using Pointer = CLBlastBackend::pointer_type<T>;
  ...
  /** Return this backend. */
  CLBlastBackend& get_backend() { return backend_; }
